...
export function loadLiveStream() {
            const platform = document.getElementById('stream-platform-select').value;
            const streamId = document.getElementById('stream-id-input').value.trim();
            const container = document.getElementById('stream-embed-container');

            if (!streamId) return showToast("Please enter a stream ID, URL, or channel name.");
